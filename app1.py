...
def validate_file(file_path):
    ...
    for row in non_english_rows:
        ...
        for col in invalid_columns:
            validation_results['non_english_chars'].append({'row': row + 2, 'column': col, 'value': df.at[row, col]})
    ...
    if not duplicate_gtis.empty:
        for gti in duplicate_gtis['GTI'].unique():
            ...
            validation_results['duplicate_gtis'].append({'GTI': gti, 'rows': [row + 2 for row in duplicate_rows]})

    # Rule 4: Validate Country and Language are numerical and not empty (vectorized)
    invalid_country_language_mask = ~(df['Countries'].apply(lambda x: isinstance(x, (int, float))) & df['Languages'].apply(lambda x: isinstance(x, (int, float))))
    invalid_country_language_rows = df[invalid_country_language_mask].index.tolist()
    for row in invalid_country_language_rows:
        validation_results['invalid_country_language'].append({'row': row + 2, 'Countries': df.at[row, 'Countries'], 'Languages': df.at[row, 'Languages']})
    ...
    for row in invalid_age_rating_rows:
        validation_results['invalid_age_rating'].append({'row': row + 2, 'Age Rating ID': df.at[row, 'Age Rating ID']})

    # Rule 6: Validate Date format (vectorized)
    date_format = re.compile(r'^\d{2}/\d{2}/\d{4}$')
    invalid_date_mask = ~df['Rating Date'].astype(str).str.match(date_format)
    invalid_date_rows = df[invalid_date_mask].index.tolist()
    for row in invalid_date_rows:
        validation_results['invalid_date_format'].append({'row': row + 2, 'Rating Date': df.at[row, 'Rating Date']})
